...
from datasets.cityscapes import CityscapesDataSet, CityscapesTrainInform, CityscapesValDataSet, CityscapesTestDataSet
from datasets.camvid import CamVidDataSet, CamVidValDataSet, CamVidTrainInform, CamVidTestDataSet
...
def build_dataset_train(dataset, input_size, batch_size, train_type, random_scale, random_mirror, num_workers):
    ...
    inform_data_file = os.path.join('./datasets/inform/', dataset + '_inform.pkl')
    ...
    if dataset == "cityscapes":
        ...
        return datas, trainLoader, valLoader
                       ...
                       # dataset is cityscapes folder
def build_dataset_test(dataset, num_workers, none_gt=False):  # Originally none_gt = False for validation
    data_dir = os.path.join('./datasets/', dataset)  # ./datasets/cityscapes
    dataset_list = os.path.join(dataset, '_trainval_list.txt')   # ./datasets/cityscapes_trainval_list.txt
    test_data_list = os.path.join(data_dir, dataset + '_test' + '_list.txt')   # '_val' '_test' for evaluation on val. and test set, respectively.
    inform_data_file = os.path.join('./datasets/inform/', dataset + '_inform.pkl')
